...
public class HugeDemo extends JFrame implements Runnable {
  ...
  public static final String[] floorDoms = {"floor.1", "floor.2", "floor.3", "floor.4", "floor.5", "floor.6"};
  ...
  public static final String[] dchosts = {"datacenter|192.168.50|!Authoritative_Database",
                                          ...
                                          "datacenter|192.168.51|Web05",
                                          "datacenter|192.168.51|Web06",
                                          "datacenter|192.168.51|Web07",
  ...
  public static final String[] sysadmins = {"Jane", "Eve", "Quorra", "Aang", "Edward", "Bernice", "Bill", "Lodi", "Maxwell", "Rachael"};
  ...
  public void run () {
    ...
    Thread t = new Thread(new StreamMonitor(this, 80000, 20000, 5000, 1000, datacenterHosts, sysHosts));
    t.start();

    Thread t3 = new Thread(new StreamMonitor(this, 8000, 500, 10000, 3000, externalHosts, internalHosts));
    t3.start();

    Thread t4 = new Thread(new StreamMonitor(this, 12000, 1000, 1200, 300, datacenterHosts, internalHosts));
    t4.start();

    Thread t7 = new Thread(new StreamMonitor(this, 20000, 1000, 900, 100, datacenterHosts, edHosts));
    t7.start();

    Thread t5 = new Thread(new StreamMonitor(this, 6000, 1000, 2600, 4000, allHosts, allHosts));
    t5.start();

    Thread t6 = new Thread(new StreamMonitor(this, 40000, 5000, 800, 200, datacenterHosts, edHosts));
    t6.start();
  ...
  private void monitor () throws IOException, InterruptedException {
	...
	if ( lastPing - start > 20000 && lastPing - start < 90000 )
  ...
  class SpitMonitor implements Runnable {
    ...
    public SpitMonitor ( HugeDemo da ) {
      this.da = da;
    }
  ...
  class StreamMonitor implements Runnable {
    ...
    public StreamMonitor ( HugeDemo da, int min, int add, long minDuration, long ranDuration,
  ...
  class TrickleMonitor implements Runnable {
    ...
    public TrickleMonitor ( HugeDemo da, String from, String to ) {
      this.da = da;
      this.from = from;
      this.to = to;
    }
